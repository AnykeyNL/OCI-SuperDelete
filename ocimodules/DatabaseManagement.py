...
def DisableDatabaseManagement(config, signer, Compartments):
            ...
            if len(items) > 0:
                for i in items:
                    if i.deployment_type == "AUTONOMOUS":
                        ...
                        try:
                            db.disable_autonomous_database_management(autonomous_database_id=i.id)
                    ...
                    if i.deployment_type == "VM" or i.deployment_type == "BM" or i.deployment_type == "EXADATA":
                        ...
                        try:
                            db.disable_database_management(database_id=i.id)
                        except oci.exceptions.ServiceError as response:
                            print("error {}-{} trying to disable: {}".format(response.code, response.message, "database management"))

                wait = True
                while wait:
                    print("Waiting for managements services to be disabled, Compartment {}               ".format(Compartment.name), end="\r")
                    items = dbm.list_managed_databases(compartment_id=Compartment.id).data.items
                    if len(items) == 0:
                        wait = False
                    time.sleep(2)
    ...
    # Catch Exception if Error
        ...
        print(f'\nError in DisableDatabaseManagement: {str(e)}')
