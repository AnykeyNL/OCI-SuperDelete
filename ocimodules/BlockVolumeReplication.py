                ...
                for v in volumes:
                    if v.volume_group_replicas is not None:
                        try:
                            ...
                            object.update_volume_group(volume_group_id=v.id, update_volume_group_details=updatedetails, retry_strategy=oci.retry.DEFAULT_RETRY_STRATEGY)
                            wait = True
                            while wait:
                            ...
                            if err.code == 404:
                                ...
                                print("error {}-{} trying to delete: {}".format(err.code, err.message, "volume group replication"))
                ...
                for v in volumes:
                    if v.block_volume_replicas is not None:
                        try:
                            ...
                            object.update_volume(volume_id=v.id, update_volume_details=updatedetails, retry_strategy=oci.retry.DEFAULT_RETRY_STRATEGY)
                            wait = True
                            while wait:
                            ...
                            if err.code == 404:
                                ...
                                print("error {}-{} trying to delete: {}".format(err.code, err.message, "block volume replication"))
                ...
                for v in volumes:
                    if v.boot_volume_replicas is not None:
                        try:
                            ...
                            object.update_boot_volume(boot_volume_id=v.id, update_boot_volume_details=updatedetails, retry_strategy=oci.retry.DEFAULT_RETRY_STRATEGY)
                            wait = True
                            while wait:
                            ...
                            if err.code == 404:
                                ...
                                print("error {}-{} trying to delete: {}".format(err.code, err.message, "boot volume replication"))

    # Catch Exception if Error
    except Exception as e:
        print(f'\nError in RemoveReplication: {str(e)}')
